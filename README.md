...
## Tech stack
Next.js 14 (App Router) · TypeScript · Zod · Vitest · Gemini 2.5 Flash (optional prose assist).

## Setup & run
1. `npm install`
2. Copy `.env.example` to `.env.local` and set `GEMINI_API_KEY` (optional — the service runs fully without it).
...
## AI approach
Rules-first: a deterministic core decides every scored field (transaction match, evidence verdict, case type, severity, routing, escalation) and enforces safety + schema. Gemini 2.5 Flash only polishes prose and is fully optional (deterministic templates are the floor).
...
## MODELS
- gemini-2.5-flash — hosted by Google, called via @google/generative-ai. Chosen for low latency and cost; used only for optional prose polishing with a deterministic fallback, so the service never hard-depends on it. No model is bundled or run locally.

## Cost reasoning
At most one short Gemini call per request; skipped entirely when no API key is present. Optional caching avoids repeat calls.
